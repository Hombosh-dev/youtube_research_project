...
df_sample <- df %>% filter(video_id %in% sample_videos)
...
df_sample$trending_next_day <- as.numeric(df_sample$trending_next_day)
...
# Розмір датасету
cat("Number of rows in dataset:", nrow(df), "\n")

# Підрахунок відео за статусом наступного дня
table_trending <- table(df$trending_next_day)
table_trending
...
ggplot(df, aes(x=comment_rate, y=likes_ratio)) +
...
```{r}
# Let`s find a
mean_of_comments = mean(df$comment_rate)
mean_of_likes = mean(df$likes_ratio)


a = sum((df$comment_rate - mean_of_comments)*(df$likes_ratio - mean_of_likes))/sum((df$comment_rate - mean_of_comments)^2)

a0 = mean_of_likes * 1000 - a*mean_of_comments* 1000

cat("a = ", a, "\n")
cat("a0 = ", a0 , "\n")
lm_fit <- lm(likes_ratio ~ comment_rate, data=df)

ggplot(df, aes(x = comment_rate*1000, y = likes_ratio*1000)) +
  geom_point(alpha = 0.3) +
...
# Підготовка даних
X_original <- df$comment_rate
M_original <- df$likes_ratio
Y <- df$trending_next_day
