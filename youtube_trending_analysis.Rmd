...
We visualize the distribution of comment counts on a logarithmic scale to handle the wide range of values and identify patterns in engagement.
...
**Dataset Summary & Trending Status**

We report the dataset size and calculate the proportion of videos that trended the next day versus those that did not.
...
**Comment Rate vs Likes Ratio**

We explore the relationship between comment rate and likes ratio through a scatterplot to assess potential correlation between these engagement metrics.
...
The regression summary indicates a strong and highly significant linear association: the estimated slope $a = 2.0264$ and intercept $a_0 = 24.496$ both have p-values \< 2e-16, confirming they are statistically different from zero. The model explains approximately 22% of the variance in the likes ratio (Adjusted $R^2 = 0.2276$), showing that comment rate is an important predictor but not the sole driver. The residual standard error of 29.25 suggests moderate dispersion around the fitted line, consistent with the large variability inherent in user-engagement metrics.
...
is a first-order Taylor approximation of the true log-likelihood around the current parameter values. Thus, IRLS repeatedly solves a **weighted** least squares system for $(c', b, \alpha_0)$ using weights $w$ and pseudo-response $u$, updating the centered weighted sums and recomputing the coefficients until convergence.
...
cat("c_prime (unscaled)=", c_prime_original, "\n")
cat("b (unscaled) =", b_original, "\n")
cat("intercept_Y =", intercept_Y, "\n\n")
# Mediation effects
IE <- (a) * (b_original)
DE <- c_prime_original
...
cat("Comparison with glm() (scaled params)\n")
...
The estimated linear model for the mediator gives M = a X + a0 with a = 2.026463 and a0 = 24.49604 indicating that each unit increase in the comment rate is associated with an average increase of approximately 2.02 units in the likes ratio. In the logistic regression stage, the IRLS algorithm converged after 8 iterations, producing the scaled coefficients c' = 0.006750642 and b = -0.003377362, where "scaled" refers to the fact that X and M were internally rescaled by 1000 to improve numerical stability of the iterations. After returning to the original units of measurement, these values become c' = 6.750642 and b = -3.377362, while the intercept remains unchanged at -5.109986 because it already reflects the original data scale.

The indirect mediation effect equals IE = a × b = -6.844097, revealing that the pathway X → M → Y slightly suppresses the overall effect, since the product a × b is negative. The direct effect is DE = c' = 6.750642 (in unscaled form), indicating that increases in comment rate directly raise the log-odds of trending, independent of likes. The agreement between the manually estimated coefficients and the glm() output confirms the correctness of the IRLS procedure, as the numbers match exactly: (Intercept = -5.109985, X = 0.006750642, M = -0.003377362).

Finally, the prediction statistics show that typical predicted probabilities of trending are extremely small: the median and mean probabilities are both about 0.00555, while the maximum predicted value is 0.1050727. This aligns with the empirical share of trending videos in the dataset (0.005550239), demonstrating that the fitted model captures the strong class imbalance and produces probabilities consistent with real-world frequencies.
...
X_org <- X/1000
M_org <- M/1000
eta_pred <- c_prime_original * (X_org) + b_original * (M_org) + intercept_Y
p_pred <- 1 / (1 + exp(-eta_pred))

# Binning
x_bins <- cut(X_org, breaks = 20, include.lowest = TRUE)

x_bin_mid <- tapply(X_org, x_bins, function(v) mean(v, na.rm = TRUE))
...
Below, we resample the dataset with replacement, refit both mediator and outcome models for each bootstrap sample, and compute the product $\hat{a}*\hat{b}$ for every iteration.\
...
The code above performs a bootstrap test of the indirect effect a × b in a mediation framework, using a computationally efficient subsampling strategy. In each bootstrap iteration, we resample a subset of the data with replacement, refit both the mediator model M \~ X and the outcome model Y \~ X + M, and compute the product of the estimated coefficients to obtain a bootstrap estimate of the indirect effect. This procedure produces an empirical sampling distribution of a × b, from which we derive the percentile-based 95% confidence interval and the two-sided bootstrap p-value. In our results, the observed indirect effect is -6.844097, while the bootstrap confidence interval [-34.71569, 25.43527] contains zero, indicating that the indirect effect is not statistically distinguishable from zero. The bootstrap p-value equals 0.649, meaning that the observed indirect effect could plausibly arise under the null hypothesis of no mediation. These results were obtained using b_original calculated on the unscaled (normal) data, which gives a realistic estimate of the indirect effect without artificially inflating or shrinking the coefficients due to data scaling.

**For balanced sample** To correct the strong class imbalance in the original dataset, we construct a balanced sample by identifying the indices of all positive cases (idx_1) and all negative cases (idx_0).\
...
X_bal <- X_org[idx_bal_fixed]
M_bal <- M_org[idx_bal_fixed]
...
The observed indirect effect a*b in the balanced dataset equals -6.965486, indicating that the mediated influence of X on Y through M is small. 
The bootstrap confidence interval [-22.88199, -1.56153] lies entirely below zero but remains relatively close to it, suggesting that the magnitude of the indirect effect varies substantially across bootstrap samples and does not provide stable evidence of a meaningful mediation effect. 
A bootstrap p-value of 0.5765 means that more than half of the bootstrap replications produced an indirect effect larger in absolute value than the observed one, implying that the estimate is not statistically distinguishable from noise. 
Because both a_bal_fixed and b_bal_fixed were estimated on a dataset with equal numbers of Y = 0 and Y = 1 cases, the absence of significance cannot be attributed to class imbalance. Instead, it reflects a genuinely weak or unstable mediated relationship. 
Therefore, the balanced-sample bootstrap analysis supports the conclusion that mediation through M is negligible, and any effect of X on Y is likely direct or too small to be reliably detected with the current modeling approach.


### 5. Conclusion

The mediation analysis of our dataset reveals that the indirect effect of comment rate (X) on next-day trending probability (Y) through likes ratio (M) is small in magnitude. Using both the unbalanced and balanced bootstrap procedures, we observe that the estimated indirect effect a × b is close to zero:

Unbalanced sample: IE = -6.760537, 95% CI 
[-34.71569 ,  25.43527 ], bootstrap p-value = 0.649

Balanced sample: IE = -6.965486, 95% CI
[-22.88199 ,  -1.56153 ], bootstrap p-value = 0.5765

These results indicate that the mediated pathway X → M → Y is not statistically significant once we account for class imbalance and the variability inherent in the data. In particular, the balanced-sample analysis demonstrates that even with equal representation of trending and non-trending cases, the indirect effect remains negligible, and the null hypothesis H0: a × b = 0 cannot be rejected.

The direct effect c', representing the influence of comment rate on trending probability independent of likes, remains positive but relatively small, consistent with the low overall probability of videos trending the next day (median predicted P(Y=1) ≈ 0.00576). The IRLS logistic regression captures the extreme class imbalance, but predictions are largely constrained to the low probability range, reflecting limited predictive power of engagement metrics for next-day trending.

Overall, our analysis suggests that while comment rate modestly affects likes ratio, this pathway does not meaningfully mediate the probability of a video trending the following day. The model’s limited predictive accuracy and low explained variance imply that additional factors—such as video content, publishing time, or external promotion—likely drive momentum in trending behavior beyond the engagement metrics considered here.
